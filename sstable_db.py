...
unfiltered = construct.Struct(
    ...
    "row" / construct.If(
        ...
        construct.Struct(
            ...
            "cells" / construct.RepeatUntil(lambda x, lst, ctx: ctx._io.tell()-ctx.row_body_start+1 >= ctx.serialized_row_body_size, simple_cell), # https://github.com/apache/cassandra/blob/cassandra-3.0/src/java/org/apache/cassandra/db/rows/BufferCell.java#L211
        ),
    ),
)
partition_header = construct.Struct(
    ...
    "key" / construct.Switch(lambda ctx: ctx._root._.sstable_statistics.serialization_header.partition_key.name, {
        b"org.apache.cassandra.db.marshal.UTF8Type": text_cell_value,
        b"org.apache.cassandra.db.marshal.Int32Type": int_cell_value,
    }),
