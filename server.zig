...
const FrameHeader = packed struct {
    ...
    fn fromBytes(
        ...
        switch (endian) {
            .little => {
                self.version = std.mem.bytesAsValue(@TypeOf(self.version), buf[0..1]).*;
                self.flags = std.mem.bytesAsValue(@TypeOf(self.flags), buf[1..2]).*;
                self.stream = @byteSwap(std.mem.bytesAsValue(i16, buf[2..4]).*);
                self.opcode = std.mem.bytesAsValue(@TypeOf(self.flags), buf[4..5]).*;
